...
class Customer(Base):
    ...
    isDeletedAt = Column("is_deleted_at", String(datetime.datetime),
                            nullable=True, default=datetime.datetime.now())
    firebaseToken = Column("firebase_token", String())
...
class CustomerMail(Base):
    ...
    _id = Column("id", UUID(as_uuid=True), primary_key=True)
    customerId = Column("customer_id", ForeignKey("customers.id"))
    email = Column(String(100), nullable=True)
    verifiedAt = Column("verified_at", String(datetime.datetime),
                           nullable=True, default=datetime.datetime.now())
    linkExpiredAt = Column(
        "link_expired_at", String(datetime.datetime), nullable=True, default=datetime.datetime.now())
    status = Column(Integer())
    emailSent = Column("email_sent", Boolean(), nullable=True)
