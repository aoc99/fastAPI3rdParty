...
from api.customer.models import CustomerModel as custModel
        ...
        # try:
            ...
            if key_args:
                if token.checkClientId(key_args):
                    ...
                    customer = jsonable_encoder(db.query(custModel).filter(
                        custModel.phoneNumber == phoneNumber, custModel.isDeleted == False).first())
                    print(customer)
                    # if customer:
