...
class ServiceCustomer:
    def __init__(self, db, dataReq, request):
        super().__init__()
        self.token = Tokenizer()
        self.dataReq = dataReq
        self.request = request

    def checkpasscode(dataCommon):
        # try:
            key_args = dataCommon.request.query_params['key']
            if key_args:
                if dataCommon.token.checkClientId(key_args):
                    data = json.loads(dataCommon.token.decrypt(data))
                    print(data)
