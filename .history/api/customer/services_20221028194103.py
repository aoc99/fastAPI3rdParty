                ...
                if self.token.checkClientId(key_args):
                    ...
                    if customer:
                        if customer.failedPasscode < 3:
                            ...
                            db.session.commit()
                            HTTPResponse.validation_error(
                                "Akun Anda Ditangguhkan, Silahkan menggunakan fitur Lupa PIN")
                    else:
                        HTTPResponse.not_found("Nasabah tidak terdaftar")
                else:
                    HTTPResponse.unauthorized("Unknown Client")
            else:
                HTTPResponse.forbidden("Tidak ada Key Otorisasi")
        except Exception as err:
            db.rollback()
            HTTPResponse.server_error(str(err))
        finally:
            db.close()
