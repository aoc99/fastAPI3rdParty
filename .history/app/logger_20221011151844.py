...
class TimedRoute(APIRoute):
    def get_route_handler(self) -> Callable:
        ...
        async def custom_route_handler(request: Request) -> Response:
            apm = make_apm_client(
                            {'SERVICE_NAME': 'middleware', 'SERVER_URL': 'http://10.6.226.246:8200'})
            app = FastAPI()
            ...
            apm.begin_transaction(
                transaction_type="script", trace_parent=parent)
            # Do some work
            apm.end_transaction(name=__name__, result="success")
            ...
            parent = elasticapm.trace_parent_from_headers(
                jsonable_encoder(requests._body))
            apm.begin_transaction(
                transaction_type="script", trace_parent=parent)
            # Do some work
            apm.end_transaction(name=__name__, result="success")
