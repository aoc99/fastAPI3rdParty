...
class Tokenizer:
    ...
    def createCustomerObject(self, customerId):
        customer = Customer.query.filter(Customer._id==customerId).first().serialize()
        if customer["profilePicture"]:
            customer["profilePicture"] = self.encrypt(customer["profilePicture"])
        email = CustomerMail.query.filter(CustomerMail.customerId==customerId).first()
        if email:
            email = email.serialize()
            customerObject = {**customer, **{"email": email["email"], "emailStatus": email["status"]}}
        else:
            customerObject = {**customer, **{"email": None, "emailStatus": None}}
    ...
    def decrypt(self, data):
        ...
        unpadded_string = unpad(decode_string, cipher.block_size).decode("utf8").strip()
